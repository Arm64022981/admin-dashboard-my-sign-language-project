  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/doctors/${editingDoctor.doctor_id}`,
        {
          ...
          body: JSON.stringify({
            ...
            contact_number: editingDoctor.contact_number,
      ...
      if (res.ok) {
        setDoctors(
          doctors.map((d) =>
            d.doctor_id === editingDoctor.doctor_id ? { ...d, ...editingDoctor } : d
          )
        );
  ...
  const handleDelete = async (doctorId: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        MySwal.fire("ลบสำเร็จ", "ข้อมูลหมอถูกลบแล้ว", "success");
      } else {
        const data = await res.json();
        MySwal.fire(
          "เกิดข้อผิดพลาด",
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center space-x-4 mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-800">รายชื่อหมอ</h1>
              <p className="text-gray-600">จัดการข้อมูลแพทย์ในระบบ</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-white rounded-xl shadow-md p-6 border border-blue-100">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">จำนวนหมอทั้งหมด</p>
            ...
            <div className="bg-white rounded-xl shadow-md p-6 border border-green-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">{filteredDoctors.length}</p>
                </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gradient-to-r from-blue-500 to-indigo-600">
                <tr>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-white">รหัสหมอ</th>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm font-medium text-gray-900">{doctor.fullname}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm text-gray-600">{doctor.contact_number}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
          ...
          {filteredDoctors.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-sm text-gray-500">
                ไม่พบหมอที่ตรงกับคำค้นหา "{searchTerm}"
              </p>
            </div>
          )}
        </div>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="grid grid-cols-4 items-center gap-4">
                    <Label htmlFor="doctors_id" className="text-right text-sm font-medium text-gray-700">
                      รหัสหมอ
                    </Label>
                    <Input
                      id="doctors_id"
                      value={editingDoctor.doctor_id}
                      disabled
